...
def njtaprocess():
    ...
    input_path = f"C:\\Users\\{username}\\Documents\\py\\input2_files\\"
    output_path = f"C:\\Users\\{username}\\Documents\\py\\output2_files\\"
    ...
    for fname in files:
        ...
        with open("text.txt", "w") as file:
            ...
            all_re = re.findall(r"(\w+\W+\d+)\s+\W+(\w+)\W+(\w+)\s+(.*)\s+(\d+\W+\d+\W+\d+\s+\d+\W+\d+\W+\d+)\s+\W+(\d+\W+\d+)\s+\W+(\d+\W+\d+)|(\w+\W+\d+)\s+\W+(\w+)\W+(\w+)\s+(.*)\s+(\d+\W+\d+\W+\d+\s+\d+\W+\d+\W+\d+)\s+\W+(\d+\W+\d+)\s+(\d+\W+\d+)|(\w+\W+\d+)\s+\W+(\w+)\W+(\w+)\s+([\w\- ]*)\W+(\d{2}\/\d{2}\/\d{2}\W*\d{2}:\d{2}:\d{2})\s{1}[$I1]{1}([\d.]*)\s{1}[$18]{1}([\d.]*)|(\w+\W+\d+)\s+\W+(\w{2})\w(\w{7})\s+([\w\- ]*)\s(\d{2}\/\d{2}\/\d{2}\W*\d{2}:\d{2}:\d{2})\s+\W+(\d+\W+\d+)\s+\W+(\d+\W+\d+)|(\w+\W+\d+)\s+\W+(\w+)\W+(\w+)\s+(.*)\s+((\d{2}\/\d{2}\/\d{2}\W*\d{2}:\d{2}:\d{2}))\s{1}[$IS]{1}([\d.]*)\s{1}[$S8I]{1}([\d.]*)",text)
            ...
            for line_data in all_re:
                ...
                rowDict["TOLL AGENCY"] = "NEW JERSEY TURNPIKE AUTHORITY"
